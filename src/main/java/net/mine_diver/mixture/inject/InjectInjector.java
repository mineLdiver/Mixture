...
public final class InjectInjector implements Injector {
    ...
    @Override
    public void inject(ClassNode mixedClass, MethodNode mixedMethod, Map<MixtureInfo.HandlerInfo, Set<AbstractInsnNode>> handlerInfos) {
        handlerInfos.forEach((handlerInfo, abstractInsnNodes) -> abstractInsnNodes.forEach(injPoint -> {
            ...
            insns.add(new MethodInsnNode(isStatic ? INVOKESTATIC : INVOKEVIRTUAL, mixedClass.name, handlerInfo.methodNode.name, Type.getMethodDescriptor(Type.VOID_TYPE, handlerArgs)));
            if (usesCallbackInfo) {
                insns.add(new VarInsnNode(ALOAD, callbackInfoVar));
            ...
            if (handlerInfo.annotation.<AnnotationInfo>get("at").getEnum("shift", At.Shift.UNSET) == At.Shift.AFTER)
                mixedMethod.instructions.insert(injPoint, insns);
            else
                mixedMethod.instructions.insertBefore(injPoint, insns);
        }));
    }
}
